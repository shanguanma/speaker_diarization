...
from model import SpeakerDiarization

import warnings
warnings.filterwarnings("ignore")


def train(configs, gpus, test_folder=None):
    ...
    trainer = pl.Trainer(
        ...
        callbacks=callbacks,
        #gpus=gpus,
        devices=gpus, ##The devices to use. Can be set to a positive number (int or str), a sequence of device indices
...
if __name__ == "__main__":
    ...
    with open(setup.configs, "r") as f:
        ...
        f.close()
    # Read config
    #with open(args.configs, 'r') as fin:
    ...
    train(configs, gpus=setup.gpus, test_folder=setup.test_from_folder)
