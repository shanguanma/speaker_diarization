...
# model config is from https://www.modelscope.cn/damo/speech_diarization_sond-zh-cn-alimeeting-16k-n16k4-pytorch.git
...
from device_funcs import force_gatherable
from label_smoothing_loss import LabelSmoothingLoss, SequenceBinaryCrossEntropy
from utils import int2vec
...
from resnet34_encoder import ResNet34Diar, ResNet34SpL2RegDiar
from conv_encoder import ConvEncoder
from ci_scorers import DotScorer, CosScorer
from self_attention_encoder import SelfAttentionEncoder
from fsmn_encoder import FsmnEncoder


class DiarSondModel(nn.Module):
    """

    `Speaker Overlap-aware Neural Diarization for Multi-party Meeting Analysis`
    (https://arxiv.org/pdf/2211.10243)
    """
    def __init__(
        self,
        token_list: str="data/token_list/powerset_label_n16k4.txt",
        encoder: torch.nn.Module=ResNet34Diar,
        speaker_encoder: torch.nn.Module=ConvEncoder,
        ci_scorer: torch.nn.Module=DotScorer,
        cd_scorer: torch.nn.Module=SelfAttentionEncoder,
        decoder: torch.nn.Module=FsmnEncoder,
        ...
        self.encoder=ResNet34Diar(input_size=80) # 80-dimension fbank feature
        self.speaker_encoder=ConvEncoder()
        self.ci_scorer=DotScorer()
        self.cd_scorer=SelfAttentionEncoder()
        self.decoder=FsmnEncoder()
        ...
        self.token_list = token_list
        vocab_size = len(self.token_list) # PSE labels have 2,517 possible categories.
...
if __name__ == "__main__":
    ...
    token_list="data/token_list/powerset_label_n16k4.txt"
    model = DiarSondModel(token_list=token_list)
    #model = DiarSondModel(token_list=token_list,encoder=ResNet34Diar,speaker_encoder=ConvEncoder,ci_scorer=DotScorer,cd_scorer=SelfAttentionEncoder,decoder=FsmnEncoder,)
    print(model)
    model_file='exp/speech_diarization_sond-zh-cn-alimeeting-16k-n16k4-pytorch/sond.pth'
    model_dict = torch.load(model_file, map_location='cpu')
    model.load_state_dict(model_dict)
    model_name_pth="exp/speech_diarization_sond-zh-cn-alimeeting-16k-n16k4-pytorch/sond_md.pth"
