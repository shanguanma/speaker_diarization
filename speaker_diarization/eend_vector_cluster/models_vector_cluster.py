...
import padertorch.base as pbase
import padertorch.summary.tbx_utils as psummary
"""
...
class PadertorchModel(pbase.Model):
...
class TransformerDiarization(nn.Module):
    def __init__(self,
        ...
        if int(in_size)!= int(n_units):
            self.proj = nn.Linear(in_size,n_units)
        else:
            self.proj = None
        self.enc_layer = nn.TransformerEncoderLayer(d_model=n_units,nhead=n_heads,dropout=dropout_rate)
        self.enc = nn.TransformerEncoder(self.enc_layer, num_layers=n_layers)
