#!/usr/bin/env python3
# Modified by: Duo MA from https://github.com/nttcslab-sp/EEND-vector-clustering/blob/main/eend/pytorch_backend/train.py
...
import torch
from speaker_diarization.feature import get_input_dim
from speaker_diarization.eend_vector_cluster.models_vector_cluster import fix_state_dict
from speaker_diarization.eend_vector_cluster.models_vector_cluster import PadertorchModel
from speaker_diarization.eend_vector_cluster.models_vector_cluster import TransformerDiarization
from speaker_diarization.eend_vector_cluster.lr_scheduler import NoamScheduler

from speaker_diarization.eend_vector_cluster.diarization_dataset_vector_cluster import DiarizationDatasetFromWave
from speaker_diarization.eend_vector_cluster.diarization_dataset_vector_cluster import DiarizationDatasetFromFeat

import padertorch.train.hooks as phooks
import padertorch.train.trainer as ptrainer
...
def train(args):
    ...
    if args.optimizer == 'noam':
        ...
        trainer.register_hook(
            phooks.LRSchedulerHook(scheduler, trigger=(1, 'iteration'))
            )

    trainer.train(trainloader, resume=True, device=device)
    print('Finished!')
