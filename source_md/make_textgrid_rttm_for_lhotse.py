...
def main(args):
    ...
    for i in range(len(segments)):
        ...
        #pdb.set_trace()
        # for matching lhoste wav.scp uttid format, I will modify uttid and remove speaker
        uttid_new = segments[i].uttid.split("_")[:-1]
        print(uttid_new)
        uttid_new = '_'.join(uttid_new)
        print(uttid_new)
        rttm_file.write(f"{fmt.format(uttid_new, float(segments[i].stime), float(segments[i].etime) - float(segments[i].stime), str(segments[i].spkr))}\n")
