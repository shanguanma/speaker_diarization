...
# copy and modified from https://github.com/BUTSpeechFIT/DiariZen/blob/main/diarizen/pipelines/inference.py
import argparse
import os
...
class DiariZenPipeline(SpeakerDiarizationPipeline):
    def __init__(
        ...
        logging.info(f"Diarizen model raw config: {config}")
        if config_parse is not None:
            logging.info("Overriding with parsed config.")
        ...
        print(
            f"inference seg_duration: {inference_config['seg_duration']} from class DiariZenPipeline"
        )
        super().__init__(
            config=config,
        ...
        if clustering_config["method"] == "AgglomerativeClustering":
            ...
            raise ValueError(
                f"Unsupported clustering method: {clustering_config['method']}"
            )
    ...
    def from_pretrained(
        ...
        diarizen_hub = snapshot_download(
            repo_id=repo_id, cache_dir=cache_dir, local_files_only=cache_dir is not None
        )
    ...
    def __call__(self, in_wav, sess_name=None):
        assert isinstance(
            in_wav, (str, ProtocolFile)
        ), "input must be either a str or a ProtocolFile"
        in_wav = in_wav if not isinstance(in_wav, ProtocolFile) else in_wav["audio"]

        logging.info("Extracting segmentations.")
...
if __name__ == "__main__":
    ...
    # Required paths
    parser.add_argument(
        "--in_wav_scp", type=str, required=True, help="Path to wav.scp."
    )
    parser.add_argument(
        "--diarizen_hub",
        type=str,
    ...
    for audio_file in audio_f:
        ...
        logging.info(f"Prosessing: {sess_name}")
