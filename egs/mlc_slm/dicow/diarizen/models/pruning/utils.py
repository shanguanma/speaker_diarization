...
from diarizen.models.module.wav2vec2.utils.import_huggingface_wavlm import (
...
def configure_optimizers(
    ...
    main_params = [
        p for n, p in model.student_model.named_parameters() if "log_alpha" not in n
    ]
    pgs = [
        {
            "params": main_params,
            ...
            [
                {
                    "params": [
                        p
                        for n, p in model.student_model.named_parameters()
                        if "log_alpha" in n
                    ],
                    "lr": reg_lr,
                    "weight_decay": 0.0,
...
def convert_wavlm(hf_dir: str, output_dir: str):
    assert (
        "base" in hf_dir or "large" in hf_dir
    ), "hf_dir must contain 'base' or 'large'"
    model_name = os.path.basename(
        os.path.normpath(hf_dir)
    )  # e.g. wavlm-base, wavlm-base-plus, wavlm-large
