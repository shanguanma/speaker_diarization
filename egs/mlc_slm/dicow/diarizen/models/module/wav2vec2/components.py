...
class FeatureExtractor(Module):
    ...
    def get_num_macs_and_final_out_channels_length(self):
        ...
        num_macs = 0
        for layer in self.conv_layers:
            layer_macs, in_channels, T_in = layer.get_num_macs_and_out_channels_length(
                in_channels, T_in
            )
            num_macs += layer_macs
...
class SelfAttention(Module):
    ...
    def get_num_macs(self, T_in):
        if self.hard_concrete_for_heads is not None:
            ...
            num_heads = self.num_heads
        num_macs = (
            4 * T_in * num_heads * self.embed_dim * self.head_dim
...
class Encoder(Module):
    ...
    def get_num_macs(self, in_features, T_in):
        """Calculate the current model size."""
        feature_projection_macs = self.feature_projection.get_num_macs(
            in_features, T_in
        )
