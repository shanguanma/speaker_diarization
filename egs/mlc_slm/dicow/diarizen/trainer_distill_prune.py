...
class Trainer:
    def __init__(
        ...
        self.sparsity_warmup_epochs = self.trainer_config.get(
            "sparsity_warmup_epochs", 5
        )

        self.target_sparsity_cur_epoch = self.trainer_config.get(
            "target_sparsity_cur_epoch", 0
        )
        self.expected_sparsity_cur_epoch = self.trainer_config.get(
            "expected_sparsity_cur_epoch", 0
        )
    ...
    def create_lr_one_cycle_scheduler(self, max_steps):
        ...
        self.lr_one_cycle_scheduler = self.accelerator.prepare(
            self.lr_one_cycle_scheduler
        )
    ...
    def train(self, train_dataloader: DataLoader, validation_dataloader):
        ...
        self.sparsity_warmup_updates = (
            self.sparsity_warmup_epochs * update_steps_per_epoch
        )
