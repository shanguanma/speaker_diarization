# pylint: skip-file
# Copied from: https://huggingface.co/BUT-FIT/DiCoW_v3_2/blob/main/decoding.py
...
class CTCPrefixScore(object):
    ...
    def __init__(self, x, blank, eos):
        ...
        self.max_num_labels = x.shape[
            2
        ]  # Set to a max value that can be dynamically resized
        self.r = torch.full(
            (self.batch_size, self.input_length, 2, self.max_num_labels),
            self.logzero,
            device=self.device,
        )
        self.xs = torch.full(
            (self.batch_size, self.input_length, self.max_num_labels),
            self.logzero,
            device=self.device,
        )
    ...
    def initial_state(self):
        ...
        r = torch.full(
            (self.batch_size, self.input_length, 2), self.logzero, device=self.device
        )
    ...
    def _compute_log_phi(self, r_sum, cs, last, decoded_len, r_prev):
        ...
        log_phi = torch.where(
            (non_zero_mask.unsqueeze(1) & label_match_mask)[:, None, :],
            r_prev[..., 1:2],
            log_phi,
        )
        return log_phi
    ...
    def _compute_log_psi(self, decoded_len, log_phi, x_current):
        ...
        start = torch.clamp(
            decoded_len, min=1
        )  # Ensure start is at least 1 to avoid out-of-bounds
        ...
        mask_t = torch.arange(1, T, device=decoded_len.device).expand(
            B, T - 1
        ) >= decoded_len.unsqueeze(1)

        # Accumulate log_psi only up to the last valid time step for each sequence
        log_psi = torch.logaddexp(
            log_psi,
            torch.logsumexp(
                torch.where(
                    mask_t.unsqueeze(-1), log_phi[:, :-1] + self.xs[:, 1:], self.logzero
                ),
                dim=1,
            ),
        )
        ...
        for t in range(start.min(), self.input_length):
            ...
            self.r[:, t, 0] = (
                torch.logaddexp(self.r[:, t - 1, 0], log_phi[:, t - 1]) + self.xs[:, t]
            )
            self.r[:, t, 1] = (
                torch.logaddexp(self.r[:, t - 1, 0], self.r[:, t - 1, 1])
                + x_current[:, t, self.blank][:, None]
            )
            if ~should_decode.any():
                self.r[:, t] = torch.where(
                    should_decode.unsqueeze(-1).unsqueeze(-1),
                    self.r[:, t],
                    self.logzero,
                )
    ...
    def __call__(self, y, cs, decoded_len, samples_to_be_decoded, r_prev):
        ...
        self.xs = torch.gather(
            x_current, 2, cs.unsqueeze(1).expand(-1, self.input_length, -1)
        )
...
class CTCRescorerLogitsProcessor(LogitsProcessor):
    def __init__(
        ...
        self.ctc_state_prev, self.ctc_score_prev = (
            self.ctc_prefix_scorer.initial_state()
        )
        ...
        self.tmp_ctc_scores = torch.empty(
            (self.batch_size, self.num_tokens - 1), device=self.device
        )
        self.tmp_ctc_states = torch.empty(
    ...
    def analyze_predictions(
        self, scores, ctc_scores, next_token_scores, input_ids, k=10
    ):
        print("\n" + "#" * 100)
    ...
    def __call__(
        self, input_ids_orig: torch.LongTensor, scores: torch.FloatTensor
    ) -> torch.FloatTensor:
        ...
        if (input_ids[:, 0] != self.bos_token_id).any():
            input_ids = torch.stack(
                [
                    row[(row == self.bos_token_id).nonzero(as_tuple=True)[0].item() :]
        ...
        decoded_len = torch.logical_and(
            input_ids <= self.first_timestamp_token_id, input_ids != self.blank_token_id
        ).sum(dim=1)
        mask = torch.logical_and(
            input_ids[:, -1] >= self.first_timestamp_token_id,
            input_ids[:, -1] != self.blank_token_id,
        )
        last_non_timestamp_token = torch.gather(
            input_ids,
            1,
            torch.logical_or(
                input_ids < self.first_timestamp_token_id,
                input_ids == self.blank_token_id,
        ...
        ids_to_score = torch.topk(
            scores[:, : self.first_timestamp_token_id], k=self.ctc_tokens_to_score
        ).indices
        ...
        self.tmp_ctc_scores[to_be_decoded] = self.tmp_ctc_scores[to_be_decoded].scatter(
            1, ids_to_score[to_be_decoded], ctc_scores_local
        )
        self.tmp_ctc_states[to_be_decoded] = (
            self.tmp_ctc_states[to_be_decoded]
            .permute(0, 2, 3, 1)
            .scatter(
                3,
                ...
                .unsqueeze(1)
                .unsqueeze(1)
                .repeat(1, *ctc_states_local.shape[1:3], 1),
                ctc_states_local,
            )
            .permute(0, 3, 1, 2)
        )
        ...
        self.tmp_ctc_scores[:, self.first_timestamp_token_id :] = (
            self.tmp_ctc_scores.max(dim=1).values[:, None]
        )
        ctc_scores = self.tmp_ctc_scores - self.ctc_score_prev

        next_token_scores = (
            1 - self.ctc_weight
        ) * scores + self.ctc_weight * ctc_scores

        if self.debug:
            self.analyze_predictions(
                scores, ctc_scores, next_token_scores, input_ids_orig
            )
...
class GreedyCTCDecoder(torch.nn.Module):
    ...
    def forward(self, emission: torch.Tensor) -> List[str]:
        ...
        indices = torch.nn.utils.rnn.pad_sequence(
            indices, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
