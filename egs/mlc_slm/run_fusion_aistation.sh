...
. /maduo/package/cuda_11.8_path.sh # # for loading library libcudnn_ops_infer.so.8 , whisperx need to it.
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ];then
   echo "prepared devset groundtruth rttm file"
...
if [ ${stage} -le 15 ] && [ ${stop_stage} -ge 15 ];then
   echo "compute DER using md_eval_22 "
   input_dir=/maduo/datasets/mlc-slm/dev/
   dest_dir=/maduo/exp/asr_sd/fusion/whisper_v3_align_model_wav2vec2_asr_large_lv60k_960h_pyannote_sd_v3.1/English
   for line in American  Australian  British  Filipino  Indian;do
          meeteval-der md_eval_22 -r  $input_dir/English/${line}_ref.stm -h $dest_dir/$line/hyp.stm --collar .25
   done
#nohup bash run_fusion.sh --stage 15 --stop-stage 15 > logs/run_fusion_stage15_compute_DER.log 2>&1 &

#grep -r "INFO %DER:" logs/run_fusion_stage15_compute_DER.log
#INFO %DER: 30.48% [ 2081.50s / 6828.23s, 209.74s missed, 938.38s falarm, 933.39s spk error ]
#INFO %DER: 16.12% [ 1179.01s / 7312.55s, 375.63s missed, 213.36s falarm, 590.01s spk error ]
#INFO %DER: 21.78% [ 1536.39s / 7052.82s, 236.97s missed, 362.70s falarm, 936.71s spk error ]
#INFO %DER: 15.86% [ 1049.43s / 6615.44s, 216.91s missed, 274.25s falarm, 558.27s spk error ]
#INFO %DER: 11.65% [ 808.49s / 6940.15s, 271.03s missed, 252.24s falarm, 285.23s spk error ]

fi

if [ ${stage} -le 16 ] && [ ${stop_stage} -ge 16 ];then
   echo "compute standard WER "
   input_dir=/maduo/datasets/mlc-slm/dev/
   dest_dir=/maduo/exp/asr_sd/fusion/whisper_v3_align_model_wav2vec2_asr_large_lv60k_960h_pyannote_sd_v3.1/English
   for line in American  Australian  British  Filipino  Indian;do
          meeteval-wer wer -r  $input_dir/English/${line}_ref.stm -h $dest_dir/$line/hyp.stm 
   done
fi

# Perform ASR reasoning based on the data generated in stage 4, 5, and 6 to ensure that each sentence ID is unique,
# which is required by the meeteval-wer tool.
if [ ${stage} -le 17 ] && [ ${stop_stage} -ge 17 ];then
   echo "inference segment audio base groundtruth metadata"
   input_dir=/maduo/datasets/mlc-slm/dev/
   for line in American  Australian  British  Filipino  Indian;do
	   grep -r "$line" $input_dir/wav.scp > $input_dir/English/${line}_wav.scp
   done
   dest_dir=/maduo/exp/asr_sd/fusion/whisper_v3_align_model_wav2vec2_asr_large_lv60k_960h_pyannote_sd_v3.1/English/only_asr
   dev_data_dir=$input_dir
   for line in American  Australian  British  Filipino  Indian;do
    mkdir -p $dest_dir/English/$line
     awk '{print $2}'  $dev_data_dir/English/${line}_wav.scp |  while read subline;do
     whisperx $subline \
           --model large-v3 \
           --align_model WAV2VEC2_ASR_LARGE_LV60K_960H \
           --batch_size 4\
