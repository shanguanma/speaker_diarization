...
def get_speaker_model(args):
    
    global _speaker_model, pretrained_state
    if _speaker_model is None:
        try:
            logging.info("初始化speaker模型...")
            ...
            logging.info(f"speaker model path: {pretrained_model}")
            pretrained_state = torch.load(pretrained_model, map_location='cpu')
            
            model = conf['model']
            _speaker_model = dynamic_import(model['obj'])(**model['args']) 

            logging.info("speaker 模型初始化成功")
        except Exception as e:
            logging.error(f"speaker 模型初始化失败: {e}")
            return None
    return _speaker_model,pretrained_state
...
def extract_embeddings(args,batch):
    ...
    # load model
    speaker_model, pretrained_state = get_speaker_model(args)
    ...
    # compute embedding
    embeddings = speaker_model.forward(batch.to(device))#(B,D)
