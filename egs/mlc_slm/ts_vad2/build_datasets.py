...
class TSVADDataConfig:
    ...
    #lid: str="American"
    #""" from American  Australian  British  Filipino  Indian  """
cfg = TSVADDataConfig()
...
def load_dataset(
    ...
    elif cfg.dataset_name == "mlc_slm": 
        if split == "dev": 
            spk_path=f"{cfg.spk_path}/{split}/dev_english/{cfg.speaker_embedding_name_dir}" ## speaker embedding directory
            json_path=f"{cfg.data_dir}/{split}/dev_english/dev_english.json"## offer mixer wavform name
            audio_path = f"{cfg.data_dir}/{split}/dev_english/target_audio" ## offer number of speaker, offer mixer wavform name, offer target speaker wav
        elif split == "train":
             spk_path=f"{cfg.spk_path}/{split}/train_english/{cfg.speaker_embedding_name_dir}" ## speaker embedding directory
             json_path=f"{cfg.data_dir}/{split}/train_english/train_english.json"## offer mixer wavform name
             audio_path = f"{cfg.data_dir}/{split}/train_english/target_audio" ## offer number of speaker, offer mixer wavform name, offer target speaker wav

        else: # infer case, split is subset of dev, i.e. American  Australian  British  Filipino  Indian
             spk_path=f"{cfg.spk_path}/{split}/{cfg.speaker_embedding_name_dir}" ## speaker embedding directory
             json_path=f"{cfg.data_dir}/{split}/dev.json"## offer mixer wavform name i.e. /maduo/datasets/mlc-slm/dev/English/American/dev.json 
             audio_path = f"{cfg.data_dir}/{split}/target_audio" ## offer number of speaker, offer mixer wavform name, offer target speaker wav i.e. /maduo/datasets/mlc-slm/dev/English/American/target_audio
        print(f"json_path: {json_path}, spk_path: {spk_path}, audio_path: {audio_path}")
    else:
