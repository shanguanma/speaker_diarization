...
def main(args):
    ...
    for uttid in wav2scp.keys():
        ...
        # if multi channel, it will convert first channel
        orig_audio, _ = soundfile.read(wav_file)
        if torch.from_numpy(orig_audio).dim()==2:# multi channel
            print(f"it is multi channel audio, select first channel!")
            orig_audio = orig_audio[:,0]
        elif  torch.from_numpy(orig_audio).dim()==1: # mono channel
            print(f"it is mono channel audio!")
