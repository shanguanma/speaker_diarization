...
from datasets_multi import load_dataset
from datasets_multi import TSVADDataConfig
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="The probability to select a batch from the alimeeting dataset",
    )
...
def add_data_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--spk-path",
        type=str,
        default="/data/maduo/model_hub/ts_vad/spk_embed/magicdata-ramc/SpeakerEmbedding",
    ...
    parser.add_argument(
        "--data-dir",
        type=str,
        default="/data/maduo/exp/speaker_diarization/ts_vad2/data/magicdata-ramc",  # oracle target audio and labels path
        help="path to target audio and mixture labels root directory.",
    )
    parser.add_argument(
        "--alimeeting-spk-path",
        type=str,
        default="/data/maduo/model_hub/ts_vad/spk_embed/alimeeting/SpeakerEmbedding",
        help="target speaker embedding path",
    )
    ...
    parser.add_argument(
        "--alimeeting-data-dir",
        type=str,
        default="/data/maduo/datasets/alimeeting",  # oracle target audio and labels path
        help="path to target audio and mixture labels root directory.",
    )
    parser.add_argument("--dataset-name",type=str,default="alimeeting+magicdata-ramc",help="dataset name", )
...
def add_data_model_common_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=192,
        help="target speaker model output feature dimension and is also tsvad speech encoder output feature dimension i.e.CAM++ speaker embed dim ",
    )
    parser.add_argument(
        "--rs-len",
        type=int,
...
def is_magicdata_ramc(batch)->bool:
    """
    I use magicdata-ramc file prefixes are CTS as a feature to determine whether the current batch is magicdata-ramc
    you can see ts_vad2/ts_vad_dataset.py and how to prepare samples?
    """
    filename = batch['net_input']["file_path"]
    #logging.info(f"filename: {filename}, it is list.")
    return filename[0].startswith("CTS")
...
def train_one_epoch(
    ...
    alimeeting_train_dl: torch.utils.data.DataLoader,
    valid_dl: torch.utils.data.DataLoader,
    rng: random.Random,
    model_avg: Optional[nn.Module] = None,
) -> None:
    ...
    Args:
      ...
      train_dl:
        Dataloader for the magicdata-ramc training dataset.
      alimeeting_train_dl:
        Dataloader for the alimeeting training dataset.
      valid_dl:
        Dataloader for the validation dataset.
    """
    model.train()
    tot_loss = MetricsTracker()
    # index 0: for magicdata
    # index 1: for alimeeting
    # This sets the probabilities for choosing which datasets
    dl_weights = [1 - params.alimeeting_prob, params.alimeeting_prob]
    iter_magicdata = iter(train_dl)
    iter_alimeeting = iter(alimeeting_train_dl)

    batch_idx = 0
    train_batch_nums = []
    while True:
        idx = rng.choices((0, 1), weights=dl_weights, k=1)[0]
        dl = iter_magicdata if idx == 0 else iter_alimeeting

        try:
            batch = next(dl)
        except StopIteration:
            name = "magicdata" if idx == 0 else "alimeeting"
        ...
        if batch_idx % params.log_interval == 0:
            ...
            logging.info(
                ...
                f"batch_idx_train: {params.batch_idx_train}, num_updates: {num_updates}, {loss_info}, "
                f"batch size: {batch_size}, grad_norm: {grad_norm}, grad_scale: {grad_scale}, "
                f"lr: {cur_lr:.2e}, {'magicdata' if is_magicdata_ramc(batch) else 'alimeeting'}"
            )
        ...
        if batch_idx == len(train_dl) - 1:
            ...
            logging.info(
                ...
                f"batch_idx_train: {params.batch_idx_train}, {loss_info}, "
                f"batch size: {batch_size}, grad_norm: {grad_norm}, grad_scale: {grad_scale}, "
                f"lr: {cur_lr:.2e}, {'magicdata' if is_magicdata_ramc(batch) else 'alimeeting'}"
            )
...
#
#                do_save_and_remove_once(
#                    params, model, model_avg, optimizer, scheduler, train_dl, scaler,valid_info["DER"],
#                )
            model.train()
            logging.info(
                f"[Eval] - Epoch {params.cur_epoch}, batch_idx_train: {params.batch_idx_train}, "
...
def main(args):
    ...
    data_cfg.magicdata_spk_path = params.spk_path
    data_cfg.alimeeting_spk_path = params.alimeeting_spk_path
    data_cfg.speaker_embedding_name_dir = params.speaker_embedding_name_dir
    data_cfg.magicdata_data_dir = params.data_dir
    data_cfg.alimeeting_data_dir= params.alimeeting_data_dir
    ...
    if params.dataset_name=="alimeeting+magicdata-ramc":
        #if params.dataset_name.split("+")[0]=="alimeeting":
        alimeeting_valid_dataset = load_dataset(data_cfg, "Eval","alimeeting")
        alimeeting_train_dataset = load_dataset(data_cfg, "Train","alimeeting")
        #if params.dataset_name.split("+")[-1]=="magicdata-ramc":
    ...
    valid_dl = DataLoader(
        ...
        collate_fn=magicdata_valid_dataset.collater,
        sampler=None,
    )

    alimeeting_train_dl = DataLoader(
        dataset=alimeeting_train_dataset,  # the dataset instance
        batch_size=params.batch_size,  # automatic batching
        drop_last=False,  # drops the last incomplete batch in case the dataset size is not divisible by 64
        # shuffle=(train_sampler is None),                 # shuffles the dataset before every epoch
        shuffle=True,
        collate_fn=alimeeting_train_dataset.collater,
        sampler=None,
    )


    magicdata_train_dl = DataLoader(
        dataset=magicdata_train_dataset,  # the dataset instance
        batch_size=params.batch_size,  # automatic batching
        drop_last=False,  # drops the last incomplete batch in case the dataset size is not divisible by 64
        # shuffle=(train_sampler is None),                 # shuffles the dataset before every epoch
        shuffle=True,
        collate_fn=magicdata_train_dataset.collater,
        sampler=None,
    )
    ...
    model, optimizer, magicdata_train_dl, alimeeting_train_dl,valid_dl = accelerator.prepare(
        model, optimizer, magicdata_train_dl, alimeeting_train_dl, valid_dl
    )
...
#    scheduler = accelerator.prepare(scheduler)
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        train_one_epoch(
            ...
            scheduler=scheduler,
            scaler=scaler,
            train_dl=magicdata_train_dl,
            alimeeting_train_dl=alimeeting_train_dl,
            valid_dl=valid_dl,
            accelerator=accelerator,
            model_avg=model_avg,
            rng=rng,
