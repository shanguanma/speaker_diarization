...
class SimAM_ResNet34_ASP(nn.Module):
    def __init__(self, in_planes=64, embed_dim=256, acoustic_dim=80, dropout=0,speech_encoder=False):
        super(SimAM_ResNet34_ASP, self).__init__()
        self.front = SimAM_ResNet34(in_planes)
        if not speech_encoder:# for tsvad diarization
    ...
    def forward(self, x,get_time_out=False):
        ...
        if get_time_out: # for tsvad diarization
            #x(B,C,F,T) it is from https://github.com/wenet-e2e/wespeaker/blob/master/wespeaker/models/pooling_layers.py#L79
                                     # https://github.com/wenet-e2e/wespeaker/blob/master/wespeaker/models/pooling_layers.py#L122
                                     # https://github.com/wenet-e2e/wespeaker/blob/master/wespeaker/models/pooling_layers.py#L167
            x = out.reshape(x.shape[0], x.shape[1] * x.shape[2], x.shape[3])
            assert len(x.shape) == 3 #(B,D,T)
            #out = out.permute(0,2,1) # (B,D,T) -> (B,T,D)
            return x           # D is feature dimension, T is time-dimension (frame-dimension)
                               # D is 5120 for SimAM_ResNet34_ASP
...
class SimAM_ResNet100_ASP(nn.Module):
    ...
    def forward(self, x,get_time_out=False):
        ...
        if get_time_out: # for tsvad diarization
            ...
            #out = out.permute(0,2,1) # (B,D,T) -> (B,T,D)
            return x           # D is feature dimension, T is time-dimension (frame-dimension)
                               # D is 5120 for SimAM_ResNet100_ASP
        x = self.pooling(x)
        if self.drop:
            x = self.drop(x)
        x = self.bottleneck(x)
...
if __name__ == '__main__':
    ...
    model2 = SimAM_ResNet34_ASP(embed_dim=256,speech_encoder=True)
    model2.eval()
    out2 = model2(x,get_time_out=True)
    print(out2[-1].size())
    print(out2.size())#torch.Size([1, 512, 10, 25]), its subsample rate is 8(200/25=8.0)
