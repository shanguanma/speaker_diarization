...
from ts_vad_dataset_multi import TSVADDataset
from dataclasses import dataclass
import logging

@dataclass
class TSVADDataConfig:
    alimeeting_data_dir: str = "/data/maduo/datasets/alimeeting"
    """path to target audio and mixture labels root directory."""

    magicdata_data_dir: str = "/data/maduo/exp/speaker_diarization/ts_vad2/data/magicdata-ramc"
    """path to target audio and mixture labels root directory."""
    ...
    alimeeting_spk_path: str = (
        "/data/maduo/model_hub/ts_vad/spk_embed/alimeeting/SpeakerEmbedding"
    )
    """path to target speaker embedding directory"""
    
    magicdata_spk_path: str = ("/data/maduo/model_hub/ts_vad/spk_embed/magicdata-ramc/SpeakerEmbedding/"
...
def load_dataset(
    ...
    if cfg.dataset_name == "alimeeting+magicdata-ramc" and dataname=="alimeeting":
        if split == "Test" or split == "Eval":
            spk_path = f"{cfg.alimeeting_spk_path}/{split}/{cfg.speaker_embedding_name_dir}"  ## speaker embedding directory
            json_path = f"{cfg.alimeeting_data_dir}/{split}_Ali/{split}_Ali_far/{split}.json"  ## offer mixer wavform name,
            audio_path = f"{cfg.alimeeting_data_dir}/{split}_Ali/{split}_Ali_far/target_audio"  ## offer number of speaker, offer mixer wavform name, offer target speaker wav,
        elif split == "Train":
            spk_path = f"{cfg.alimeeting_spk_path}/{split}/{cfg.speaker_embedding_name_dir}"  ## speaker embedding directory
            json_path = f"{cfg.alimeeting_data_dir}/{split}_Ali_far/{split}.json"  ## offer mixer wavform name,
            audio_path = f"{cfg.alimeeting_data_dir}/{split}_Ali_far/target_audio"  ## offer number of speaker, offer mixer wavform name, offer target speaker wav,
    
    elif cfg.dataset_name == "alimeeting+magicdata-ramc" and dataname=="magicdata-ramc":
        spk_path=f"{cfg.magicdata_spk_path}/{split}/{cfg.speaker_embedding_name_dir}"  ## speaker embedding directory
        json_path=f"{cfg.magicdata_data_dir}/{split}/{split}.json"  ## offer mixer wavform name,
        audio_path = f"{cfg.magicdata_data_dir}/{split}/target_audio" ## offer number of speaker, offer mixer wavform name, offer target speaker wav
        #logging.info(f"spk_path: {spk_path}")
