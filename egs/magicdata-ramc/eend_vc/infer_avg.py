...
def diarize_session(
    ...
    logger.info('Extracting segmentations...')
...
if __name__ == "__main__":
    ...
    #parser.add_argument("-ckpt-dir", type=str, help="experiment directory, for example:exp/wavlm_updated_conformer_magicdata-ramc/ ")
    ...
    if args.val_metric_summary:
        ...
        if args.val_mode == "best":
            logger.info(f'averaging the best {args.avg_ckpt_num} checkpoints...')
            segmentation = val_metric_lst_sorted[:args.avg_ckpt_num]
        elif args.val_mode == "prev":
            logger.info(f'averaging previous {args.avg_ckpt_num} checkpoints to the converged moment...')
            segmentation = val_metric_lst[
                best_val_metric_idx - args.avg_ckpt_num + 1 :
                best_val_metric_idx + 1
            ]
        else:
            logger.info(f'averaging {args.avg_ckpt_num} checkpoints around the converged moment...')
            segmentation = val_metric_lst[
    ...
    if args.uem_file:
        ...
        for sess in sess2audio.keys():
            logger.info("Diarizing Session {}".format(sess))
            if args.uem_file:
                c_uem = uem_map[sess]
            else:
                c_uem = None
