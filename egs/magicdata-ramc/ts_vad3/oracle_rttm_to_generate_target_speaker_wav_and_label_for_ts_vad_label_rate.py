...
def main(args):
    ...
    for uttid in wav2scp.keys():
        ...
        id_full = uttid
        print(f"label_rate: {int(args.label_rate)}!!!")
        for key in new_intervals:
            ...
            new_audio = []
            labels = [0] * int(length / 16000 * int(args.label_rate))  # 15ms, one label, its hop_lenght=240, label_rate=67; 10ms, one label, its hop_length=160, label_rate=100; 40ms, one label, its downsample rate is 4, label_rate=25
            for interval in new_intervals[key]:
                s, e = interval
                for i in range(int(s * int(args.label_rate)), min(int(e * int(args.label_rate)) + 1, len(labels))):
                    labels[i] = 1
                s *= 16000
                e *= 16000
...
def get_args():
    ...
    parser.add_argument("--label_rate", type=int, default=67, help="The number of tokens corresponding to one second of audio, i.e. offical ReDimNetB2 melbank feature is 1s = 67 frames, its label rate is 67")
