...
class TSVADDataset(torch.utils.data.Dataset):
    ...
    def comput_audio_feat(self,path: str) -> torch.Tensor:
            ...
            if aux_len <= self.ts_len:
                ...
                if self.is_train:
                    start_frame = np.int64(
                        random.random() * (aux_len - self.ts_len) * self.sample_rate
                    )
                else:
                    start_frame = 0
                if not self.is_train:
                    target_speech, _ = self.read_audio_with_resample(
                        path,start=start_frame,length=int(self.ts_len * self.sample_rate), sr_cur=self.sample_rate
                    )
                else:
                    target_speech, _ = self.read_audio_with_resample(
                        path,
                        start=start_frame,
                        length=int(self.ts_len * self.sample_rate),
                        sr_cur=self.sample_rate
                    )

            target_speech = torch.FloatTensor(np.array(target_speech))
            # compute fbank feat
            target_feat = self.feature_extractor(target_speech)
        return target_feat # (T,D)

    def load_ts_feat(self,file, speaker_ids):
        target_feats = []
        exist_spk = []
        ...
        for speaker_id in speaker_ids:
            if speaker_id == -1: # Obatin the labels for silence
                if np.random.choice(2, p=[1 - self.zero_ratio, self.zero_ratio]) == 1 or not self.is_train:
                    ...
                    feature = torch.zeros(1, self.fbank_dim) # speaker embedding dimension of speaker model
                    #print(f"feature shape: {feature.shape} in  speaker_id == -1 zero_ratio")
                else:
                    ...
                    path = os.path.join(self.audio_path,
                            f"{random.choice(self.spk2data[random_spk])}.wav",
                        )
                    ## for
                    feature = self.comput_audio_feat(path) #
                    #print(f"feature shape: {feature.shape} in  speaker_id == -1")
            elif speaker_id == -2: # # Obatin the labels for extral
                feature = torch.zeros(2,self.fbank_dim) # feat dimension of self.feature_extractor
                #print(f"feature shape: {feature.shape} in  speaker_id == -2")
            else: # # Obatin the labels for speaker
                audio_filename=speaker_id
                path = os.path.join(self.audio_path, file, str(audio_filename) + ".wav")
                feature = self.comput_audio_feat(path)
                #print(f"feature shape: {feature.shape} in  speaker_id == {speaker_id}")

            target_feats.append(feature)
            #print(f"target_feats len: {len(target_feats)},target_feats: {target_feats}")
        #target_feats = torch.stack(target_feats)
        #print(f"target_feats shape: {target_feats.shape}")
        target_feats = _collate_data(target_feats, is_embed_input=True) #(len(max_num_speaker),T.self.fbank_dim)
        #print(f"target_feats shape: {target_feats.shape}")
        return target_feats, len(speaker_ids)
    ...
    def load_ts_embed_variable_number_speakers(self, file, speaker_ids):
        target_speeches = []
        exist_spk = []
        #print(f"file:{file}, speaker_ids: {speaker_ids}")
        for speaker_id in speaker_ids:
    ...
    def load_ts_embed(self, file, speaker_ids):
        if self.dataset_name == "alimeeting" or self.dataset_name == "magicdata-ramc":
            ...
            if self.support_variable_number_speakers:
                target_speeches, num_speakers = self.load_ts_embed_variable_number_speakers(file, speaker_ids)
            else:
                target_speeches, num_speakers = self.load_ts_embed(file, speaker_ids)
    ...
    def __getitem__(self, index):
        ...
        if self.spk_path is None:
            if self.fbank_input:
                target_speech,num_speaker = self.load_ts_feat(file, new_speaker_ids)
            else:
