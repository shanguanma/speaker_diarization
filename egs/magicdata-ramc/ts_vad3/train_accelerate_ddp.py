...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--fusion-type", type=str, default="att_wo_linear", help="""choice it from `att_wo_linear, att_w_linear`""")
    parser.add_argument("--without-speaker-utt-embed", type=str2bool, default=False, help="""""")
    parser.add_argument("--fusion-case", type=str, default="",help="")
    return parser
...
def get_optimizer(params, model):
    from torch.optim import AdamW
    # (MADUO)check if module has parameters, when ddp mode, it don't support module hasn't parameters
...
def main(args):
    ...
    model_cfg.fusion_type = params.fusion_type
    ...
    model_cfg.without_speaker_utt_embed=params.without_speaker_utt_embed
    model_cfg.fusion_case = params.fusion_case
    ...
    if params.speech_encoder_type!="w2v-bert2":
        ...
        if True:
            model.speech_encoder.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
            model.speaker_encoder.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
