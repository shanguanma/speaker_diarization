...
if [ ${stage} -le 150 ] && [ ${stop_stage} -ge 150 ];then
    ...
    speech_encoder_path="/share/workspace/maduo/model_hub/speaker_pretrain_model/w2v-bert2.0/model.safetensors"
    speech_encoder_config="/share/workspace/maduo/model_hub/speaker_pretrain_model/w2v-bert2.0/config.json"
...
if [ ${stage} -le 153 ] && [ ${stop_stage} -ge 153 ];then
    ...
    #exp_dir=/mntcephfs/lab_data/maduo/exp/speaker_diarization/ts_vad2/ts_vad2_two_gpus_freeze_with_musan_rirs_wav-bert2.0_epoch40_front_fix_seed
    exp_dir=/share/workspace/maduo/exp/speaker_diarization/ts_vad2/magicdata-ramc-ts_vad2_two_gpus_freeze_with_musan_rirs_cam++_200k_zh_cn_speaker_embed_w2v-bert2_speech_encoder_epoch40_front_fix_seed_lr1e4_single_backend_2mamba2_multi_backend_transformer_rs_len4_shift2
   #CUDA_VISIABLE_DEVICES=0,1 accelerate launch --main_process_port 12673 ts_vad2/train_accelerate_ddp2.py \
  #accelerate launch --debug --multi_gpu --mixed_precision=fp16 --num_processes=2  --main_process_port=12673 ts_vad2/train_accelerate_ddp2.py \
   data_dir="/share/workspace/maduo/datasets/MagicData-RAMC/maduo_processed/kaldi_format" # oracle target audio , mix audio and labels path
   rs_len=4
...
if [ ${stage} -le 155 ] && [ ${stop_stage} -ge 155 ];then
   echo "compute CDER for magicdata-ramc"
   threshold="0.2 0.3 0.35 0.4 0.45 0.5 0.55 0.6 0.7 0.8 0.9" # total 11
   predict_rttm_dir=/share/workspace/maduo/exp/speaker_diarization/ts_vad2/magicdata-ramc-ts_vad2_two_gpus_freeze_with_musan_rirs_cam++_200k_zh_cn_speaker_embed_w2v-bert2_speech_encoder_epoch40_front_fix_seed_lr1e4_single_backend_2mamba2_multi_backend_transformer_rs_len4_shift2
   oracle_rttm_dir=/share/workspace/maduo/datasets/MagicData-RAMC/maduo_processed/kaldi_format
   infer_sets="dev test cssd_testset"
   c=0.0
   dataset_name="magicdata-ramc"
