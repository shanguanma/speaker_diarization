...
sub evaluate {
	...
	foreach my $chnl (sort keys %{$ref_data->{$file}}) {
        ...
        #print "(maduo add,scores=$scores{SPEAKER}, file=$file )\n";
        #print "file=$file\n";
...
sub score_speaker_segments {
    ...
    #print "MISSED_SPEAKER=$stats->{FALARM_SPEAKER}\";
    #my $size=@my_miss_list;
    #$stats->{MISSED_SPEAKER_SEG_num} = push(@my_miss_list,$seg_dur*max($nref-$nsys,0)) if $seg_dur*max($nref-$nsys,0);
...
#accumulate statistics
	...
	foreach $chnl (keys %{$scores->{$file}}) {
        ...
        #print "ref_type=$ref_type in fn sd_performance_analysis\n";
...
sub print_sd_scores {
    ...
    printf "miss=%.2f, falarm=%.2f confusion=%.2f, $condition; miss_segment=$scores->{MISSED_SPEAKER},falarm=$scores->{FALARM_SPEAKER}, speaker_error=$scores->{SPEAKER_ERROR}, scored_speaker=$scores->{SCORED_SPEAKER}\n",
        100*$scores->{MISSED_SPEAKER}/$scores->{SCORED_SPEAKER}, 100*$scores->{FALARM_SPEAKER}/$scores->{SCORED_SPEAKER}, 100*$scores->{SPEAKER_ERROR}/$scores->{SCORED_SPEAKER};
