...
def get_dual_optimizer(params, model):
    from torch.optim import AdamW
    optimizer_small = AdamW(
        model.speech_encoder.parameters(),
        lr=params.lr_small,
        betas=(0.9, 0.98),
        eps=1e-08,
        weight_decay=0.01,
    ...
    optimizer_big = AdamW(
        model.non_speech_encoder_parameters(),
        lr=params.lr_big,
        betas=(0.9, 0.98),
        eps=1e-08,
        weight_decay=0.01,
...
def get_dual_scheduler(params, optimizer_small, optimizer_big, max_num_updates):
    ...
    scheduler_small = PolynomialDecayLR(
        optimizer_small, max_num_updates, params.warmup_updates, power=1.0
    )
    scheduler_big = PolynomialDecayLR(
        optimizer_big, max_num_updates, params.warmup_updates, power=1.0
    )
    return scheduler_small, scheduler_big
...
def train_one_epoch(
    params: AttributeDict,
    model: nn.Module,
    optimizer_small: torch.optim.Optimizer,
    optimizer_big: torch.optim.Optimizer,
    scheduler_small: LRSchedulerType,
    scheduler_big: LRSchedulerType,
    scaler: GradScaler,
    accelerator,
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.log_interval == 0:
            ...
            grad_scale = ""
            cur_lr_small = scheduler_small.get_last_lr()[0]
            cur_lr_big = scheduler_big.get_last_lr()[0]
            logging.info(
                ...
                f"lr_small: {cur_lr_small:.2e}, lr_big: {cur_lr_big:.2e}."
            )
...
def main(args):
    ...
    ## prepared optimizer,
    optimizer_small,optimizer_big =  get_dual_optimizer(params, model)
    ## accelerated model, optimizer, scheduler ,train_dl, valid_dl
    model, optimizer_small,optimizer_big, train_dl, valid_dl = accelerator.prepare(
        model, optimizer_small,optimizer_big, train_dl, valid_dl
    )
    ...
    scheduler_small,scheduler_big =  get_dual_scheduler(params, optimizer_small,optimizer_small, max_num_updates)
    scheduler = accelerator.prepare(scheduler_small,scheduler_big)
