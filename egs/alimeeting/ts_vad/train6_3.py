...
def get_optimizer_scheduler(params, model):
    ...
    #optimizer = AdamW(model.parameters(),lr=5e-5,betas=(0.9, 0.98)) # same as fairseq2 wav2vec2 asr train
    #from dynamic_loss_scaler2 import  DynamicLossScaler
    from polynomial import PolynomialDecayLR
    scheduler = PolynomialDecayLR(optimizer,40000,4000,power=1.0,)
...
def get_scaler():
    ...
    scaler = DynamicLossScaler(optimizer,root_gang,sharded=False,init_scale=128.0,min_scale=0.0001,gradient_accumulation=1,enabled=True)
    return scaler
