...
"""
python3 ts_vad/train6_2.py --world-size 1 --num-epochs 30 --start-epoch 1 --use-fp16 1 --exp-dir ts_vad/exp6_2
"""
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(),5.0,2.0)
        ...
        if batch_idx == params.cur_epoch *(len(train_dl) - len(train_dl) % params.log_interval) + len(train_dl) % params.log_interval:
            logging.info(
                f"end of epoch {params.cur_epoch}, "
...
def get_optimizer_scheduler(params, model):
    optimizer = torch.optim.Adam(model.parameters(), lr=2e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01)
    #scheduler = Eden(optimizer, params.lr_batches, params.lr_epochs)
    #from polynomial_decay_schedule import MyAdam
    #optimizer = MyAdam(model.parameters(), lr=2e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01)
    #from polynomial_decay_schedule import PolynomialDecayLRScheduleConfig
    from polynomial_decay_schedule import PloyDecaySchedule
    #lr_conf = PolynomialDecayLRScheduleConfig()
    scheduler = PloyDecaySchedule(optimizer)
...
def run(rank, world_size, args):
    ...
    scaler = GradScaler(enabled=params.use_fp16, init_scale=128,growth_factor=2.0,backoff_factor=0.5,growth_interval=2000)
    #from dynamic_loss_scaler import DynamicLossScaler
    #scalar = DynamicLossScaler(init_scale=128,scale_window=2**14,tolerance=0.0,threshold=None,min_loss_scale=0.0001)
