...
def extract_embeddings(fbanks, batch_size, session, subseg_cmn):
    ...
    for i in tqdm(range(0, fbanks_array.shape[0], batch_size)):
        batch_feats = fbanks_array[i:i + batch_size] # (96, 150, 80),(<96, 150, 80)
                                                     # 96 should be batch_size,
                                                     # 150 should be window_fs(1.50*1000//10),
                                                     # 80 should be fbank feature dimension.
        batch_embs = session.run(input_feed={'feats': batch_feats},
                                 output_names=['embs'])[0].squeeze() # (96, 256) or  (<96,256)


        embeddings.append(batch_embs)
    embeddings = np.vstack(embeddings) # (subsegment_nums, 256)
    print(f"np.vstack(embeddings shape: {embeddings.shape}")
    return embeddings
