...
from checkpoint import load_checkpoint, remove_checkpoints, remove_epochs
...
def do_save_and_remove_once(params,model,model_avg,optimizer,scheduler,train_dl,scaler):
    # save model
    save_checkpoint_with_global_batch_idx(
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if (
            ...
            and params.batch_idx_train % params.save_every_n == 0 and accelerator.is_main_process
        ):

            do_save_and_remove_once(params,model,model_avg,optimizer,scheduler,train_dl,scaler)
        ...
        if batch_idx > 0 and batch_idx % params.valid_interval == 0:
            ...
            model.train()
            logger.info(f"[Eval] - Epoch {params.cur_epoch}, batch_idx_train: {params.batch_idx_train-1} validation: {valid_info}")
            logger.info(
...
def main(args):
    ...
    # The logger must be placed in this location
    from accelerate.logging import get_logger
    logger = get_logger(__name__)
    # set logger using accelerator
    # only print it on main_process.
    logger.info(f"{params.exp_dir}/train.log", main_process_only=True)


    logger.info(f"params: {params}")
    logger.info(f"The scale window is set to {scale_window} in GradScaler")
    ...
    train_dl = DataLoader(dataset=train_dataset, # the dataset instance
                                 batch_size=params.batch_size, # automatic batching
                                 drop_last=False,               # drops the last incomplete batch in case the dataset size is not divisible
