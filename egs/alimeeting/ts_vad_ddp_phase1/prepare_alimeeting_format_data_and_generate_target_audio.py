            ...
            for j in range(tg[i].__len__()):
                if tg[i][j].mark:
                    if tg[i].name not in spk:
                        ...
                        num_spk += 1
                    #print(f"tg[i].name: {tg[i].name}, spk[tg[i].name]: {spk[tg[i].name]}")
                    segments.append(
                        Segment(
                            uttid,
                            spk[tg[i].name], # speaker index number in this long audio
                            tg[i][j].minTime,
                            tg[i][j].maxTime,
                            tg[i][j].mark.strip(),
                            tg[i].name, # actual speaker name in this long audio
                        )
                    )
