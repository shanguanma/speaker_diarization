...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        # it doesn't suport deepcopy
        #self.pos_conv = nn.utils.weight_norm(self.pos_conv, name="weight", dim=2)
        from torch.nn.utils.parametrizations import weight_norm # required torch version >=2.1.0
...
if __name__ == "__main__":
    ref_speech = torch.randn(64,64000)
    device = torch.device("cpu")
    speech_encoder_path= "/mntcephfs/lab_data/maduo/model_hub/speaker_pretrain_model/wavlm/WavLM-Base+.pt"
    checkpoint = torch.load(speech_encoder_path, map_location=device)
    wavlm_cfg = WavLMConfig(checkpoint["cfg"])
    print(f"wavlm_cfg origin encoder_layers: {wavlm_cfg.encoder_layers}")
    wavlm_cfg.encoder_layers = 12
    model = WavLM(wavlm_cfg)
    y = model.extract_features(ref_speech,output_layer=12,ret_layer_results=True)
    ...
    #        feature = (feature, res["layer_results"])
    #    return feature, res["padding_mask"]
    print(len(y[0])) # it is 2
    #print(len(y[0][0])) # it is 64
    print(y[0][0].shape)#(B,T,D), it is output of last layer of transformer
    #print(y[0][0][0].shape)
    print(len(y[0][1])) # when output_layer=12,ret_layer_results=True, it is 13, Because the first element of ret_layer_results is the result of pos_conv, and the second element is the output of the first layer of transformer, Each element here is actually a tuple. The first element of the tuple is x, and the second element is z.
    print(y[0][1][0]) #
    print(y[0][1][1])
    print(f"y[0][1][0][0] shape: {y[0][1][0][0].shape}") # (T,B,D)
