...
def get_args():
    ...
    parser.add_argument("--feat_type",type=str, default="utt_feat",help="choices from utt_feat, frame_feat, fbank_feat")
...
CAMPPLUS_VOX = {
    'obj': 'ts_vad3.cam_pplus_wespeaker.CAMPPlus',
    'args': {
        'feat_dim': 80,
        'embedding_size': 512,
    },
...
def extract_embeddings(args,batch):
    ...
    if args.feat_type=="fbank_feat":
        embeddings = batch
    # compute embedding
    # only for cam++ model, i modified it, and output utterance feat and frame feat
    x, framex = model.forward(batch.to(device))#x: (B,D), frame_x: (B,T,D)
    if args.feat_type=="utt_feat":
        embeddings = x
    elif args.feat_type=="frame_feat":
        embeddings = framex
