...
def add_data_model_common_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Set the length of the target speech capture (in seconds) for infer and train",
    )
    parser.add_argument(
        "--rs-len",
        type=int,
        default=4,
        help="Set the length of the reference(mixer) speech capture (in seconds) for infer and train",
    )
    parser.add_argument(
        "--rs-segment-shift",
        type=int,
        default=2,
        help="""
        Set the length of the reference speech sliding window (in seconds) for infer (i.e.rs-segment-shift=1)
        Set the length of the reference speech sliding window (in seconds) for train (i.e.rs-segment-shift=2)
        """,
    )
    return parser
