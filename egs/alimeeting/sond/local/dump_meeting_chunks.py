...
def process(feat_scp, labels_scp, spk2utt, utt2xvec, utt2frames, args):
    ...
    for mid, feat_path in tqdm(feat_scp, total=len(feat_scp), ascii=True, disable=args.no_pbar):
        ...
        logger.info(f"load label.scp: labels: {labels}, its spk list: {meeting_spk_list}")
        if args.add_mid_to_speaker:
            # # "{}_S{:03d}" is same to speker name of local/extract_nonoverlap_segments.py
            meeting_spk_list = ["{}_S{:03d}".format(mid, int(x)) if not x.startswith(mid) else x for x in meeting_spk_list]
        ...
        for i in range(num_chunk):
            ...
            utt_id = "{}-S{:04d}_{:07d}_{:07d}".format(mid,i, st, ed)
            chunk_feat = feat[st: ed, :] # shape: (frames,D), D: fbank dimension, i.e. 80
            chunk_label = labels[st: ed, :] # shape: (frames,num_speakers), num_speakers is actually max number of speaker at currently dataset,
                                            # i.e. alimeeting set , its every audio contains max number of speaker is 4.
            frame_pad = chunk_size - chunk_label.shape[0]
            spk_pad = args.n_spk - chunk_label.shape[1]
            # np.pad its pad shape [(top, down),(left,right)]
            ...
            for i, spk in enumerate(spk_list):
                ...
                logger.info(f"spk_xvec: {spk_xvec}")
