...
# This is from https://github.com/modelscope/FunASR/blob/v0.8.8/egs/alimeeting/sa_asr/local/alimeeting_data_prep.sh
...
Options:
    ...
    --alimeeting_corpus: i.e.for Eval /mntcephfs/lab_data/maduo/datasets/alimeeting/Eval_Ali/
                         i.e.for Train /mntcephfs/lab_data/maduo/datasets/alimeeting/
                         i.e. for Test /mntcephfs/lab_data/maduo/datasets/alimeeting/Test_Ali/
   For example my alimeeting corpus is as follows:
$ tree  -L 2 /mntcephfs/lab_data/maduo/datasets/alimeeting/
/mntcephfs/lab_data/maduo/datasets/alimeeting/
├── Eval_Ali
│   ├── Eval_Ali_far
│   └── Eval_Ali_near
├── Test_Ali
...
SECONDS=0
tgt=Eval #Train or Eval
alimeeting_corpus=/mntcephfs/lab_data/maduo/datasets/alimeeting/Eval_Ali/
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    python3 local/alimeeting_process_textgrid.py --path $near_dir --no-overlap False
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python3 local/alimeeting_process_overlap_force.py  --path $far_dir \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    python3 local/process_textgrid_to_single_speaker_wav.py  --path $far_single_speaker_dir
