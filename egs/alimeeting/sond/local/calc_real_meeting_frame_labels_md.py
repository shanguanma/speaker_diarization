...
class MyRunner(MultiProcessRunnerV3):
    ...
    def prepare(self, parser):
        parser.add_argument("dir", type=str) # this directory will contain ref rttm files (i.e. alimeeting_eval.rttm),wav.scp
        ...
        meeting_scp={}
        with open(os.path.join(args.dir, "wav.scp"),'r')as f:
            for line in f:
                line = line.strip().split()
                meeting_scp[line[0]] = line[-1]
    ...
    def load_rttm(self, dir_path):
        ...
        if os.path.exists(os.path.join(dir_path, "rttm.scp")):
            ...
            rttm_scp=[]
            with open(os.path.join(dir_path, "rttm.scp"),'r')as f:
                for line in f:
                    line =line.strip().split()
                    rttm.append((line[0],line[-1]))
...
def process(task_args):
    ...
    for mid, wav_path, rttms in task_list:
        meeting_labels, spk_list = build_labels(wav_path, rttms, args.n_spk, args.remove_sil,
                                                args.sample_rate, args.frame_shift)
        ...
        #save_path = os.path.join(args.out_dir, "{}.lbl.txt".format(mid))
        #with open(save_path,'w',encoding='utf-8')as f:
        #    json.dump({"labels": meeting_labels.astype(bool), "spk_list": spk_list},f, ensure_ascii=False)
        # print mid
    return None
