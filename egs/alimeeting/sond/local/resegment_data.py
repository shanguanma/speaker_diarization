...
with codecs.open(data_path + "/wav.scp", "w", "utf-8") as f1:
    with codecs.open(data_path + "/profile.scp", "w", "utf-8") as f2:
        for sessionid, wav_path in wav_dict.items():
            ...
            while flag:
                start = cur_time
                end = cur_time + 256000 # 256000/160=1600, 160 means 10ms for 16000Hz audio, 1600 *10ms = 16000ms = 16s, this windows size =16s,
                if end < wav_new.shape[0]:
                    cur_wav = wav_new[start: end]
                else:
                    cur_wav = wav_new[start: ]
                    flag = False
                cur_time = cur_time + 64000 # 64000/160=400, 400 means 25ms for 16000Hz audio, window shift size = 400*10ms = 4000ms = 4s
