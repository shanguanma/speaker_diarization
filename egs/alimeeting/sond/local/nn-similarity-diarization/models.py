...
class BiLSTMSimilarity(nn.Module):
    """
    this model is from section 2.4 of `Speaker Overlap-aware Neural Diarization for Multi-party Meeting Analysis`
    # (https://arxiv.org/pdf/2211.10243)
    its vector dimension is 256, stack two xvector ,so now the feature dimension is 512
    it is not used.
    """
...
class LSTMSimilarity(nn.Module):
    """
    this model is used at section 2.4 of `Speaker Overlap-aware Neural Diarization for Multi-party Meeting Analysis`
    # (https://arxiv.org/pdf/2211.10243)
    its vector dimension is 256, stack two xvector ,so now the feature dimension is 512
    """
    def __init__(self, input_size=512, hidden_size=256, num_layers=2):
