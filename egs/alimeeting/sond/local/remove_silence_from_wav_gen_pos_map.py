...
class MyRunner(MultiProcessRunnerV3):
    ...
    def prepare(self, parser):
        ...
        print(f"args.sr: {args.sr}")
        #meeting_scp = load_scp_as_list(os.path.join(args.dir, "reco.scp"))
        meeting_scp=[]
        with open(os.path.join(args.dir, "reco.scp"),'r')as f:
            for line in f:
                line = line.strip().split()
                meeting_scp.append((line[0],line[-1]))
...
def process(task_args):
    ...
    for mid, wav_path, vad_list in task_list:
        ...
        """
        wav = librosa.load(wav_path, sr=None, mono=True)[0] * 32767
        seg_list = []
        pos_map = []
        offset = 0
