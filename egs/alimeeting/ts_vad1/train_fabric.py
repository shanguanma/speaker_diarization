...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx > 0 and batch_idx % params.valid_interval == 0:
            ...
            model.train()
            fabric.print(f"[Eval] - Epoch {params.cur_epoch}, batch_idx_train: {params.batch_idx_train-1} validation: {valid_info}")
            fabric.print(
...
def main(args):
    ...
    #    loss, loss_info = compute_loss(model=model, batch=batch,is_training=True)
    #    model.train()
    fabric.print(f"start training from epoch {params.start_epoch}")
    fabric.print(f"Train set grouped total_num_itrs = {len(train_dl)}")
    for epoch in range(params.start_epoch, params.num_epochs+1):
        ...
        if params.batch_idx_train>=params.max_updates:
            ...
            fabric.print(f"batch_idx_train >= {params.max_updates}, stop training")
            break
    logging.info("Done!")
