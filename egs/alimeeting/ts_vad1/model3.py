...
class TSVADModel(nn.Module):
    def __init__(
        ...
        assert (
            task_cfg.speech_encoder_type == "cam++"
        ), f"task_cfg.speech_encoder_type: {task_cfg.speech_encoder_type}"
        assert (
            task_cfg.label_rate == 25
        ), f"cam++ label rate should be 25, but now is {task_cfg.speech_encoder_type}"
        # Speech Encoder
    ...
    def rs_forward(self, x, max_len):  # B, 25 * T
        B = x.size(0)
        T = x.size(1)
        #if (
        #    self.speech_encoder_type == "cam++"
    ...
    def ts_forward(self, x):  # B, 4, 80, T * 100
        logging.info(f"use_spk_embed: {self.use_spk_embed}")
        if self.use_spk_embed:
            logging.info(f"use_spk_embed: should be true")
    ...
    def forward(
        ...
        #    if name == 'speech_encoder.xvector.block3.tdnnd16.linear1.weight':
        #        logger.info(f"in the model forward model name parameter: {name}")
        outs,outs_prob = self.forward_common(
            ref_speech,
            target_speech,
            labels,
        )
        return outs, outs_prob
        # loss, outs_prob = self.calculate_loss(outs, labels, labels_len)
        #loss, _ = self.calculate_loss(outs, labels, labels_len)
        #return loss
    ...
    def forward_infer(
        ...
        #loss, result = force_gatherable((loss, result), loss.device)
        #return loss, result
    ...
    def forward_common(
        ...
        # start=None,
        # extract_features:bool=False,
        #fix_encoder: bool = True, # default set
