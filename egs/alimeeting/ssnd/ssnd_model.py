...
class DetectionDecoder(nn.Module):
    ...
    def __init__(self, d_model, nhead, d_ff, num_layers, d_aux, d_pos, out_vad_len):
        ...
        torch.nn.init.constant_(self.out_proj.bias, -2.0)  # sigmoid(-2) ≈ 0.12
...
class SSNDModel(nn.Module):
    ...
    def forward(self, feats, spk_label_idx, vad_labels, spk_labels=None):
        ...
        bce_loss = F.binary_cross_entropy_with_logits(vad_pred, vad_labels, pos_weight=pos_weight, reduction='sum')
        bce_loss = bce_loss / vad_labels.numel()  # 或 / (vad_labels > 0).sum()
