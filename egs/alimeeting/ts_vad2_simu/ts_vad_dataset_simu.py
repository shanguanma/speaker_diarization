...
class TSVADDatasetSimu(torch.utils.data.Dataset):
    def __init__(
        self,
        json_path: str,
        audio_path: str,
    ...
    def load_data_and_label(self, json_path):
        ...
        for line in tqdm(lines):
            ...
            if filename in filename_set:
                ...
                speakers = [Path(i).stem for i in audios if Path(i).stem!="all"]
                #num_speaker = (
                #    len(audios) - 1
                ...
                for start in range(0, length, dis):
                    ...
                    if end - start > self.label_rate * short_ratio:
                        data_intro = [filename, speakers, start, end]
    ...
    def get_ids(self, speakers:List[str]):
        num_speaker = len(speakers)
        speaker_ids=[0 for i in range(self.max_num_speaker)]
        for i in range(self.max_num_speaker):
            if i < num_speaker:
                speaker_ids[i] = speakers[i]
            else:
                speaker_ids[i] = -1

        #for i,speaker in enumerate(speakers):
        #    if i+1 < self.max_num_speaker:
        #        speaker_ids.append(speaker)
        #    else:
        logger.debug(f"speaker_ids: {speaker_ids},speakers: {speakers}!!!")
        if self.is_train:
            random.shuffle(speaker_ids)
        return speaker_ids
    ...
    def load_rs(self, file, speaker_ids, start, stop):
        ...
        for speaker_id in speaker_ids:
            if speaker_id == -1:
                ...
                logger.debug(f"full_label_id: {full_label_id}")
                #logger.info(f"label: {label}")
                #logger.info(f"self.label_dic: {self.label_dic}")
            ...
            logger.debug(f"new_speaker_ids: {new_speaker_ids}")
            if sum(labels[-1]) == 0 and self.is_train:
                new_speaker_ids.append(-1)
            elif (
                #sum(new_speaker_ids) != -1 * len(new_speaker_ids) # this condition means that it cannot be all -1
                len(set(new_speaker_ids))!=1
    ...
    def __getitem__(self, index):
        ...
        file, speakers, start, stop = self.data_list[index]
        speaker_ids = self.get_ids(speakers) # num_speaker means that it contains number of speaker in current mixture utterance.
