...
def main(args):
    ...
    for audio_wavform in tqdm.tqdm(audio_wavforms):
        ...
        with open(args.simu_rttm, 'r')as f:
            for line in f:
                # i.e. SPEAKER SPK0177-R0014_M0087_MS002_49_SPK0177-R0014_M0087_MS002_108_SPK0094-R0003_M0047_MS006_98_SPK4052-R1019_M4052_MS103_166 1 0.610 6.550 <NA> <NA> SPK0177-R0014_M0087_MS002_49 <NA> <NA>
                line = line.strip().split()
                if line[1] == uttid:
                    #spk_name_id = uttid + "_" + "spk" + str(line[7]) # The actual speaker name is bound to the audio name.
                    spk_seg = line[7].split("-")[-1].split("_")[-1] # 49
                    spk_name_id = line[7].split("-")[0] # SPK0177
                    spk_key = spk_name_id + '_' + spk_seg # SPK0177_49
                    ...
                    if spk_name_id not in spk:
                        spk[spk_name_id] = spk_key
                    #if spk_key not in spk:
                    #    spk[spk_key] = spk_name_id
                        #num_spk +=1
