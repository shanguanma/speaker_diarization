...
def main(args):
    ...
    for audio_wavform in tqdm.tqdm(audio_wavforms):
        ...
        with open(args.simu_rttm, 'r')as f:
            for line in f:
                # i.e. SPEAKER SPK0177-R0014_M0087_MS002_49_SPK0145-R0008_M0072_MS002_108_SPK0094-R0003_M0047_MS006_98_SPK4052-R1019_M4052_MS103_166 1 0.610 6.550 <NA> <NA> SPK0177-R0014_M0087_MS002_49 <NA> <NA>
                line = line.strip().split()
                if line[1] == uttid:
                    #spk_name_id = uttid + "_" + "spk" + str(line[7]) # The actual speaker name is bound to the audio name.
                    spk_name_id = line[7].split("-")[0] # SPK0177
                    target_speaker_speech_id = "_".join(line[7].split("_")[:-1]) # SPK0177-R0014_M0087_MS002_49 -> SPK0177-R0014_M0087_MS002
                    start = float(line[3])
                    end = start + float(line[4])
                    tgt_spkr_speech = wavscp2dict(args.target_speaker_speech_wavscp)[target_speaker_speech_id]
...
def get_args():
    ...
    #parser.add_argument("--all_seg",type=str2bool, default=False,help="if true,target speaker speech merge all segment")
