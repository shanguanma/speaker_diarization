...
from ts_vad_dataset_simu import TSVADDatasetSimu
from dataclasses import dataclass


@dataclass
class TSVADDataConfigSimu:
    data_dir: str = "/mntcephfs/lab_data/maduo/datasets/alimeeting"
    """path to target audio and mixture labels root directory."""

    eval_test_data_dir: str = "/mntcephfs/lab_data/maduo/datasets/alimeeting"
    """path to target audio and mixture labels root directory.
    It mainly serves simulated data, and you hope to use the real development set when training simulated data."""
...
def load_dataset_simu(
    ...
    if cfg.dataset_name == "alimeeting":
        if split == "Test" or split == "Eval":
            spk_path = f"{cfg.eval_test_spk_path}/{split}/{cfg.speaker_embedding_name_dir}"  ## speaker embedding directory
            json_path = f"{cfg.eval_test_data_dir}/{split}_Ali/{split}_Ali_far/{split}.json"  ## offer mixer wavform name,
            audio_path = f"{cfg.eval_test_data_dir}/{split}_Ali/{split}_Ali_far/target_audio"  ## offer number of speaker, offer mixer wavform name, offer target speaker wav,
        elif split == "Train":
