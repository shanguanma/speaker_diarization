...
# copy and modified from https://github.com/hitachi-speech/EEND/blob/master/eend/bin/rttm_stats.py
import numpy as np
import argparse
...
print(f"recordings lenght: {len(recordings)}, recording mean lenght: {np.mean(reclens)}\n")
print(f"vafs mean : {np.mean(vafs)}, duration mean : {np.mean(uds)}, ids mean: {np.mean(ids)}")
print(f"overlap ratio: {np.sum(pres[2:])/np.sum(pres[1:])},overlaps: {' '.join(str(x) for x in pres/den)}")
