...
def find_checkpoints(out_dir: Path, iteration: int = 0) -> List[str]:
    ...
    #print(f"iter_checkpoints: {iter_checkpoints} in find_checkpoints")
...
def find_epochs(out_dir: Path, epoch: int = 0) -> List[str]:
    ...
    if epoch >= 0:
        ans = [ic[1] for ic in epoch_checkpoints if ic[0] >= epoch]
    else:
        ans = [ic[1] for ic in epoch_checkpoints if ic[0] <= -epoch]
...
def remove_epochs(
    ...
    if rank != 0:
        return
    checkpoints = find_epochs(out_dir)
    ...
    logging.info(f"currently, remove unused epoch-*.pt")
    for c in to_remove:
        os.remove(c)



def find_checkpoints_and_epochs(out_dir: Path, iteration: int = 0, epoch: int = 0) -> List[str]:
    checkpoints = list(glob.glob(f"{out_dir}/checkpoint-[0-9]*.pt"))
    pattern = re.compile(r"checkpoint-([0-9]+).pt")
    iter_checkpoints = []
    for c in checkpoints:
    ...
    # for epoch
    epoch_checkpoints = list(glob.glob(f"{out_dir}/epoch-[0-9]*.pt"))
    epoch_pattern = re.compile(r"epoch-([0-9]+).pt")
    epoch_checkpoints = []
    for c in epoch_checkpoints:
        epoch_result = epoch_pattern.search(c)
        if not epoch_result:
            logging.warn(f"Invalid checkpoint filename {c}")
            continue

        epoch_checkpoints.append((int(epoch_result.group(1)), c))
    #print(f"iter_checkpoints: {iter_checkpoints} in find_checkpoints")
    # iter_checkpoints is a list of tuples. Each tuple contains
    # two elements: (iteration_number, checkpoint-iteration_number.pt)
    epoch_checkpoints = sorted(epoch_checkpoints, reverse=True, key=lambda x: x[0])
    ...
    if epoch >= 0:
        epoch_ans = [ic[1] for ic in epoch_checkpoints if ic[0] >= epoch]
    else:
        epoch_ans = [ic[1] for ic in epoch_checkpoints if ic[0] <= -epoch]

    return ans, epoch_ans


def remove_checkpoints_and_epochs(
    out_dir: Path,
    topk: int,
    rank: int = 0,
):
    ...
    if rank != 0:
        return
    checkpoints, epoch_checkpoints = find_checkpoints_and_epochs(out_dir)

    if len(checkpoints) == 0 and len(epoch_checkpoints) == 0 :
        logging.warn(f"No checkpoints found in {out_dir}")
        return

    if len(checkpoints) <= topk and len(epoch_checkpoints) <= topk:
        return
    ...
    epoch_to_remove = epoch_checkpoints[topk:]
    for c in epoch_to_remove:
        os.remove(c)
...
def remove_checkpoints(
    ...
    to_remove = checkpoints[topk:]
    logging.info(f"currently, remove unused checkpoint-*.pt")
    for c in to_remove:
        os.remove(c)
