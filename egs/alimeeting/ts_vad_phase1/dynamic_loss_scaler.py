#!/usr/bin/env python3
# Modified by Duo Ma from https://github.com/facebookresearch/fairseq2/blob/main/src/fairseq2/optim/dynamic_loss_scaler.py
        ...
        *,
        ...
        if scale_window is None:
            if enabled:
                ...
                scale_window = max(int(2**14 / int(world_size) / gradient_accumulation), 1)
                log.info(f"The scale window is set to {scale_window}.")
            else:
                scale_window = 1

        if not enabled or not sharded or int(world_size) == 1:
