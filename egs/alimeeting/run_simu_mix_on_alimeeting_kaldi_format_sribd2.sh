...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ];then
  ...
  CONV_VAL_DIR=${simu_alimeeting_dataset_dir}/${audio_amount}h_${determine_spks}_${num_spks}spks${suffix}/eval2/conv_val_dir
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ];then
   ...
   done > $data_val_dir/origin_rttm

   ## we will modify it and let spkid is real spkid
   python3 ts_vad2_simu/post_rttm.py $data_val_dir/origin_rttm > $data_val_dir/rttm
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ];then
   ...
   done > $data_train_dir/origin_rttm

   ## we will modify it and let spkid is real spkid
   python3 ts_vad2_simu/post_rttm.py $data_train_dir/origin_rttm > $data_train_dir/rttm
